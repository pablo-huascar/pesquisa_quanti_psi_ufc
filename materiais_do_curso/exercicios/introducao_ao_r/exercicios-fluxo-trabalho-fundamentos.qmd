---
title: "Introdução ao R - Exercícios"
...
Crie um vetor chamado `idades` com as idades de cinco pessoas: `25, 30, 22, 28 e 35`. Em seguida, use a função `sort()` para ordenar as idades em ordem crescente e decrescente.

## Exercício 14

Utilize a função `sample()` para gerar um vetor de 10 números inteiros aleatórios entre 1 e 50. Em seguida, calcule a média e o desvio padrão desses números. Use `set.seed(1)` para garantir que os números aleatórios gerados serão os mesmos para todos.

## Exercício 15

Crie um objeto chamado `nomes` contendo os nomes: `"Ana"`, `"Bruno"`, `"Carlos",` `"Diana"` e `"Eduardo"`. Em seguida, use a função `paste()` para combinar os nomes com seus respectivos números de identificação, que são de 1 a 5.

## Exercício 16

Use a função `factor()` para transformar o vetor sexo \<- c("M", "F", "F", "M", "F") em um fator. Em seguida, obtenha um resumo desse fator com a função `summary()`.
