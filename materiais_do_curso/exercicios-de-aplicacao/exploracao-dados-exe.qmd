...
brasileiro <- brasileiro |> 
  ...
  mutate(
    ...
    saldo_de_gols = ifelse(is.na(saldo), gols - lead(gols), saldo),
...
**Produza gráficos que avaliem a assoiação entre o resultado da partida e o mando de campo. Considere o período como um todo, bem como gere gráficos para cada campeonato**

```{r}
#| eval: false

brasileiro |> 
  ggplot(aes(mando_de_campo, 
             fill = resultado)) +
  geom_bar() +
...
brasileiro |> 
  ...
  geom_jitter() +
  geom_hline(yintercept = mean(brasileiro$gols), color = "red") +
  facet_wrap(~time)
