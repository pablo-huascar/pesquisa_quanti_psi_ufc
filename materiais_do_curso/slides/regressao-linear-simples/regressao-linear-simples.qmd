...
y_cont <- a_cont + b_cont*penguins$flipper_length_mm[1]
```

- Valor observado de body_mass_g = $`r penguins[[1,6]]`$
- Valor ajustado/previsto 
  -   $body\_mass\_g = `r a_cont` + `r b_cont`* `r penguins[[1,5]]`$
  -   $body\_mass\_g = `r y_cont`$
- $\text{Resíduo} = \text{valor observado} - \text{valor ajustado}$
  -   $\text{Resíduo} =  `r penguins[[1,6]]` - `r y_cont`  = `r penguins[[1,6]] -  y_cont`$