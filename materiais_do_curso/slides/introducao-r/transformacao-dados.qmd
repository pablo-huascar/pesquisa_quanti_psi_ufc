...
cut_ac_nutri <- ac_nutri |> 
  mutate(
    idade_corte = cut(idade, 
                      ...
                      right = T) # Padrão: inclui o valor indicado no ponto de corte <= 20
  ) |> 
  relocate(idade_corte, .after = idade)
...
Crie as variáveis `imc_corte` e `frequencia_treino_corte` no banco `acompanhamento_nutri` a partir dos pontos de corte a seguir a seguir:
...
ac_nutri |> 
  mutate(
    imc_corte = case_when(imc < 18.5 ~ "baixo peso",
