...
**6.  Calcule o limites dos intervalos: comece com o menor valor menor valor e adicione largura dos intervalos para determinar o limite superior da primeira classe**
...
tribble(
  ...
  '2. n', NA,
  '3. Mínimo', NA,
  '4. Amplitude (maior - menor)', NA,
  '5. Bins ótimos ([log2(n) + 1])', NA,
  '6. Largura das Classes (amplitude/numero de intervalos)', NA
) |> 
  datatable(
    rownames = F, editable = T,
    options = list(
  ...
  formatStyle(
    ...
    textAlign = styleEqual(c('1. Dados ordenados', 
                             '2. n', '3. Mínimo',
                             '4. Amplitude (maior - menor)',
                             '5. Bins ótimos ([log2(n) + 1])',
                             '6. Largura das Classes (amplitude/numero de intervalos)'
    ), 'left')
  ) |> 
...
```{r}
breaks_altura_da_turma <- c(152, 156, 161, 166, 171)

hist_altura_da_turma |> 
  tibble() |> 
  ggplot(aes(hist_altura_da_turma)) +
  geom_histogram(breaks = breaks_altura_da_turma, color = "#FFFFFF", 
                 fill = "#569BBD") +
  labs(
    x = "Altura da turma"
  )
...
breaks_diamonds <- pretty(range(diamonds$price),
