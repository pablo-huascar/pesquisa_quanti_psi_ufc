...
altura_2 |> 
  ...
  summarise(
    ...
    Variância = SS/(n_altura2 - 1),
    "Desvio parão" = sqrt(Variância),
  ...
  pivot_longer(cols = everything(), names_to = "Estatística",  
               values_to = "Valor") |> 
  gt() |> 
  fmt_number(columns = Valor, rows = c(1, 2, 3, 4, 6, 7), 
             decimals = 0, use_seps = F) |> 
  fmt_number(columns = Valor, rows = 5, 
             decimals = 1) |> 
  cols_align(
    columns = 2,
    align = "center"  
...
Agora gere um histograma com a nova amostra da altura dos alunos
