...
dispersao |> 
  ggplot(aes(x, y)) +
  geom_point(color = "#569BBD", alpha = 0.6,
             size = 3) +
  geom_segment(x = x, xend = x, color = "#F05133",
               y = (y-0.15), yend = 0, linetype = 2) +
  geom_segment(x = 0, xend = x-0.02, color = "#F05133",
...
father.son |> 
  ...
  geom_jitter(color = "#000000", alpha = 0.3, 
              size = 3) +
...
## [Propriedades da média \[completar\]]{style="color: red;"}

-   Senvível a valores extremos
...
2.  Conte o quantidade de dados no conjunto:

$n$ = {`r length(dados_hist)`}
...
-   $\lceil \rceil$: símbolos que significam “teto” – ou seja, arredondar a resposta para o número inteiro mais próximo.

$$
\text{Bins Ótimos} = \lceil \log_2(30) + 1 \rceil = 6
$$

5.  Largura dos intervalos: divida a amplitude (53) pelo número de classes:
...
::: {style="font-size: 60%"}
6.  Calcule o limites das classes: Comece com o menor valor menor valor e adicione largura das classes para determinar o limite superior da primeira classe
...
7.  Conte dos dados em cada intervalo:
    -   Intervalo 1 (52 - 61): 4 dados (52 53 57 59)
    -   Intervalo 2 (62 - 71): 6 dados (62, 65, 67, 68, 69, 70)
    -   Intervalo 3 (72 - 81): 6 dados (72, 75, 78, 78, 79, 80)
    -   Intervalo 4 (82 - 91): 5 dados (82, 83, 85, 88, 90)
    -   Intervalo 5 (92 - 101): 7 dados (92, 93, 95, 96, 97, 98, 100)
    -   Intervalo 6 (102 - 111): 2 dados (102 105)
...
-   Gere um histograma a partir da amostra abaixo que representa a altura dos alunos de uma turma:

```{r}
hist_altura_da_turma <- c(152, 165, 158, 170, 
                          162, 168, 155, 172, 
                          160, 163, 157, 166, 
                          159, 171, 164)
```

$\{ 152, 165, 158, 170, 162, 168, 155, 172, 160, 163, 157, 166, 159, 171, 164\}$
...
tribble(
  ...
  '2. n', NA,
  '3. Amplitude (maior - menor)', NA,
  '4. Bins ótimos ([log2(n) + 1])', NA,
  '5. Largura das Classes (amplitude/numero de classes)', NA
) |> 
  datatable(
    rownames = F, editable = T,
    options = list(
  ...
  formatStyle(
    ...
    valueColumns = 'Passos',
    target = 'cell',
    textAlign = styleEqual(c('1. Dados ordenados', '2. n',
                             '3. Amplitude (maior - menor)',
                             '4. Bins ótimos ([log2(n) + 1])',
                             '5. Largura das Classes (amplitude/numero de classes)'
    ), 'left')
  ) |> 
   formatStyle(columns = c(1, 2), fontSize = '50%')
```
...
-   Chamamos de desvio a distância de uma observação de sua média
...
Com a soma dos erros dos desvios ao quadrao ($SS$), já resolvemos a parte de cima da equção! Sabenos o número de observações, então também já temos o valor de $n$ ($n =$ `r n_prof_ze`).
...
```{r}
#| label: tbl-prof-irene
#| tbl-cap:  Avaliações da professora Irene
...
```{r}
#| label: tbl-prof-reinaldo
#| tbl-cap:  Avaliações do professor Reinaldo
#| tbl-cap-location: top
