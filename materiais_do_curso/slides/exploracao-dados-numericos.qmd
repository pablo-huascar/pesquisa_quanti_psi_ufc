...
```{r}
#| include: false
source("C:/projetos-r/pesquisa_quanti-psi-ufc/_common.R")
```
...
age_data <- read_csv("C:/projetos-r/pesquisa_quanti_psi_ufc/data/AgeDataset-V1.csv")

brasileiro <- read_csv("C:/projetos-r/pesquisa_quanti_psi_ufc/data/brasileiro_2014_2023.csv")
gols_brasileiro <- read_csv("C:/projetos-r/pesquisa_quanti_psi_ufc/data/gols_por_partida_2014_2023.csv")
```
...
## Histogramas e a distribuição dos dados {.smaller}

-   Os gráficos de pontos mostram o valor exato para cada observação
    -   Úteis para pequenos conjuntos de dados
    -   Difíceis de ler com amostras maiores

## Histogramas e a distribuição dos dados
...
airquality |> 
  ggplot(aes(Temp, fill = destacar,
             color = destacar)) +
  geom_histogram(breaks = pretty(range(airquality$Temp),
...
father.son |> 
  ggplot(aes(sheight)) +
  geom_histogram(aes(y = after_stat(density)),
                 breaks = pretty(range(father.son$sheight),
